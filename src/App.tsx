...
export default function App() {
  ...
  return (
    <div className="natural-gradient selection:bg-olive selection:text-white min-h-screen transition-colors duration-500">
      ...
      />
      {/* Navigation */}
      <nav className={`fixed top-0 left-0 w-full z-[100] p-3 md:p-5 flex justify-between items-start transition-all duration-500 ${isScrolled ? 'py-3 md:py-4' : 'py-5'}`}>
        <a href="#" className={`flex flex-col items-center gap-1.5 px-3 py-2 md:px-4 md:py-3 rounded-2xl border transition-all group backdrop-blur-md ${isScrolled ? 'bg-white/80 border-olive/10 shadow-sm' : 'bg-white/30 border-white/20'}`}>
          ...
          <span className={`font-serif text-[10px] md:text-xs font-medium tracking-tight transition-colors text-center leading-tight ${isScrolled ? 'text-natural-ink' : 'text-white'}`}>Thiago<br/>Figueiró</span>
        </a>
