...
export const Scene: React.FC<SceneProps> = ({ scene, index }) => {
  ...
  return (
    <div ref={ref} className={`relative h-screen h-[100dvh] w-full flex overflow-hidden ${index === 0 ? 'justify-center items-center lg:justify-start lg:items-end pb-20 lg:pb-24 p-4 lg:pl-20' : 'justify-center items-center'} ${!scene.image ? 'bg-[#F7F5F2]' : ''}`}>
      {scene.image && (
        <motion.div 
        ...
        >
          ...
          />
          <div className={`absolute inset-0 ${index === 0 ? 'bg-transparent lg:bg-gradient-to-r lg:from-[#1A1A1A]/40 lg:via-transparent lg:to-transparent' : (scene.id === 'identificacao' ? 'bg-black/60 backdrop-blur-[2px]' : 'bg-black/30')}`} />
        </motion.div>
      )}
      ...
      <motion.div 
        ...
        className={`relative z-10 w-full max-w-7xl px-6 md:px-12 flex items-center lg:gap-20 justify-center lg:justify-start ${index === 0 ? 'text-center lg:text-left h-full pb-32 lg:pb-0' : ''}`}
      >
        <div className={`max-w-xl lg:max-w-3xl flex flex-col ${index === 0 ? 'items-center lg:items-start' : (scene.sideImage ? 'text-left items-start' : (scene.id === 'identificacao' ? 'text-center max-w-4xl mx-auto items-center' : 'text-center items-center'))}`}>
          <div className="overflow-hidden mb-2 lg:mb-4">
            <motion.span 
              ...
              className={`block font-sans text-[10px] md:text-xs lg:text-sm uppercase tracking-[0.6em] font-black ${scene.image ? 'text-white drop-shadow-md' : 'text-primary-blue'}`}
            >
              {scene.accent}
            </motion.span>
          </div>
          ...
          {scene.subtitle && (
            <div className="overflow-hidden mb-6 lg:mb-8">
              <motion.span 
                ...
                className={`block font-sans text-xs md:text-sm lg:text-lg uppercase tracking-[0.15em] font-bold italic ${scene.image ? 'text-white/90 drop-shadow-md' : 'text-primary-blue/80'}`}
              >
                {scene.subtitle}
              </motion.span>
            </div>
          ...
          <div className="relative">
            <motion.h2 
              ...
              className={`text-4xl sm:text-5xl md:text-7xl lg:text-8xl xl:text-9xl font-serif italic ${scene.id === 'identificacao' ? 'leading-[1.1] md:leading-[1.1]' : (index === 0 ? 'leading-[0.9] lg:-ml-1' : 'leading-[1.1]')} ${scene.image ? 'text-white drop-shadow-[0_10px_30px_rgba(0,0,0,0.4)]' : 'text-natural-ink'}`}
            >
              {scene.title}
            </motion.h2>
